...
def get_attendance(id: int, db: Session = Depends(get_db)):
    record = get_attendance_by_id(db, id)
    if not record:
...
def update_existing_attendance(id: int, attendance: AttendanceUpdate, db: Session = Depends(get_db)):
    updated_record = update_attendance(db, id, attendance.dict(exclude_unset=True))
...
def delete_existing_attendance(id: int, db: Session = Depends(get_db)):
    success = delete_attendance(db, id)
    if not success:
        raise HTTPException(status_code=404, detail="Attendance record not found")
